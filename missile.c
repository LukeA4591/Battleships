...
void moveMissile(position_t* pos, uint8_t missile, uint8_t current_column) {
    ...
    if (navswitch_push_event_p (NAVSWITCH_EAST)){
        if ((pos->column < 4) && collision_check(missile, pos->column + 1, pos->row, missileMap)) {
    ...
    if (navswitch_push_event_p (NAVSWITCH_WEST)) {
        if ((pos->column > 0) && collision_check(missile, pos->column - 1, pos->row, missileMap)) {
            missileMap[pos->column - 1] |= (0x01 << pos->row);
            missileMap[pos->column] ^= (0x01 << pos->row);
            pos->column--;
        }
    ...
    if (navswitch_push_event_p (NAVSWITCH_NORTH)) {
        if ((pos->row > 0) && collision_check(missile, pos->column, pos->row - 1, missileMap)) {
            missileMap[pos->column] ^= (0x01 << pos->row);
            pos->row--;
            missileMap[pos->column] |= (0x01 << pos->row);
        }
    ...
    if (navswitch_push_event_p (NAVSWITCH_SOUTH)) {
        if ((pos->row < 6) && collision_check(missile, pos->column, pos->row + 1, missileMap)) {
            missileMap[pos->column] ^= (0x01 << pos->row);
            pos->row++;
            missileMap[pos->column] |= (0x01 << pos->row);
        }
